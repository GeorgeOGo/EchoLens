...
def extract_keyframes_and_events(video_path="output_video_preprocessing.mp4", 
    ...
    while cap.isOpened():
        ...
        if significant_event:
            ...
            if motion_score > event_motion_peak:
                ...
                print(f"Updated peak frame at frame {frame_count} with motion score {motion_score}")
            ...
            if peak_frame is not None:
                ...
                for _ in range(significant_frames_per_keyframe):
                    out_significant.write(peak_frame[0])
                saved_significant_frames += 1
                print(f"Saved significant keyframe at frame {frame_count}")
...
def generate_descriptions_and_summary(video_path="significant_keyframes_output.mp4", output_dir="frames", predicted_label=None, confidence=None):
    ...
    genai.configure(api_key="AIzaSyDRZxJYFreRuziDI1jOj384l5nENBzh20U")
