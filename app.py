...
def download_report(source):
    ...
    # Retrieve results and significant keyframes path from session
    results = session.get(results_key)
    significant_keyframes_path = session.get(significant_keyframes_key)

    if not results or not significant_keyframes_path or not os.path.exists(significant_keyframes_path):
        logger.error(f"download_report: Required data not found in session for source {source}")
        return jsonify({'error': 'Analysis results or keyframes not found'}), 404

    # Extract a keyframe image
    ...
    try:
        ...
        # Clean up the PDF file and significant keyframes video
        paths_to_remove = [pdf_path, significant_keyframes_path]
        for path in paths_to_remove:
            if path and os.path.exists(path):
                try:
                    os.remove(path)
